...
#(
  ...
  // Special case for OTBN AppIntf
  always_comb begin
    if (app_id == AppConfigDynamic) begin
      app_o[AppConfigDynamic] = '{
        ready:         app_data_ready | fsm_data_ready,
        done:          otbn_app_intf_done,
        digest_share0: {128'h0, packed_digest_word_share_0},
        digest_share1: {128'h0, packed_digest_word_share_1},
        error:         error_i | fsm_digest_done_q | sparse_fsm_error_o
                       | service_rejected_error
      };
    end else begin
  ...
  assign req_packed_digest_word = app_active_o && (app_id == AppConfigDynamic);
  ...
  assign pack_digest_word = ((prim_mubi_pkg::mubi4_test_true_strict(squeezing_i) && !squeezing_q) || shift_and_pack_digest) && ~clr_appid && (app_id == AppConfigDynamic);
