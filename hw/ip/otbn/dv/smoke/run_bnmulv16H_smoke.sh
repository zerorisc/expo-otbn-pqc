...
$OTBN_UTIL/otbn_as.py -o $SMOKE_BIN_DIR/bnmulv16H_smoke_test.o $SMOKE_SRC_DIR/bnmulv16H_smoke_test.s || \
    fail "Failed to assemble bnmulv16H_smoke_test.s"
$OTBN_UTIL/otbn_ld.py -o $SMOKE_BIN_DIR/bnmulv16H_smoke.elf $SMOKE_BIN_DIR/bnmulv16H_smoke_test.o || \
    fail "Failed to link bnmulv16H_smoke_test.o"
...
timeout 5s \
  ...
  --load-elf=$SMOKE_BIN_DIR/bnmulv16H_smoke.elf -t | tee $RUN_LOG
...
had_diff=0
grep -A 71 "Call Stack:" $RUN_LOG | diff -U3 $SMOKE_SRC_DIR/bnmulv16H_smoke_expected.txt - || had_diff=1
