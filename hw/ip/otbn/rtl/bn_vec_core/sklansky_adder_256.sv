module sklansky_adder_256 (
    ...
    generate
        ...
        for (i = 0; i < 256; i = i + 1) begin : stage_5
            if ((i % 32) >= 16) begin
                assign g_s5[i] = (word_mode != MODE_16) ? g_s4[i] | (p_s4[i] & g_s4[i - (i%32) + 15])  : g_s4[i];
                assign p_s5[i] = (word_mode != MODE_16) ? p_s4[i] & p_s4[i - (i%32) + 15]              : p_s4[i];
            end else begin
        ...
        for (i = 0; i < 256; i = i + 1) begin : stage_6
            if ((i % 64) >= 32) begin
                assign g_s6[i] = (word_mode == MODE_64) ? g_s5[i] | (p_s5[i] & g_s5[i - (i%64) + 31]) : g_s5[i];
                assign p_s6[i] = (word_mode == MODE_64) ? p_s5[i] & p_s5[i - (i%64) + 31]             : p_s5[i];
            end else begin
        ...
        for (i = 0; i < 256; i = i + 1) begin : stage_7
            if ((i % 128) >= 64) begin
                assign g_s7[i] = (word_mode == MODE_64) ? g_s6[i] | (p_s6[i] & g_s6[i - (i%128) + 63]) : g_s6[i];
                assign p_s7[i] = (word_mode == MODE_64) ? p_s6[i] & p_s6[i - (i%128) + 63]             : p_s6[i];
            end else begin
        ...
        for (i = 0; i < 256; i = i + 1) begin : stage_8
            if ((i % 256) >= 128) begin
                assign g_s8[i] = (word_mode == MODE_64) ? g_s7[i] | (p_s7[i] & g_s7[i - (i%256) + 127]) : g_s7[i];
                assign p_s8[i] = (word_mode == MODE_64) ? p_s7[i] & p_s7[i - (i%256) + 127]             : p_s7[i];
            end else begin
    ...
    assign carry[0] = (word_mode == 0) ? cin : 1'b0;
    generate
        for (i = 1; i < 256; i = i + 1) begin : carry_assign
          if ((i % 32) == 0)
            assign carry[i] = (word_mode == MODE_64) ? g_s8[i - 1] : 1'b0;
          else if ((i % 16) == 0)
            assign carry[i] = (word_mode != MODE_16) ? g_s8[i - 1] : 1'b0;
          else
    ...
    assign sum = p ^ carry;
    assign cout = (word_mode == 0) ? g_s8[255] : 1'b0;
