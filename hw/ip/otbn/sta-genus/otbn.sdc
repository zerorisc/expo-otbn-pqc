...
if {[llength [get_ports -quiet ${MAIN_CLK_PIN}]] > 0} {
  set_clock_uncertainty ${SETUP_CLOCK_UNCERTAINTY} ${MAIN_CLK_PIN}

  create_clock ${MAIN_CLK_PIN} -period ${MAIN_TCK}

  # ---- in2reg: inputs -> regs captured by clk
  set_max_delay ${MAIN_TCK} -from [get_ports [all_inputs]] -to [get_clocks ${MAIN_CLK_PIN}]
  
  # ---- reg2out: regs launched by clk -> output ports
  set_max_delay ${MAIN_TCK} -from [get_clocks ${MAIN_CLK_PIN}] -to [get_ports [all_outputs]]
}

set_max_delay ${MAIN_TCK} -from [get_ports [all_inputs]] -to [get_ports [all_outputs]]
# -datapath_only
