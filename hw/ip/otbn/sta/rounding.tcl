...
#
# Strategy 1: Grid-first (choose SMALLEST grid g >= 1%·|x|)
# Strategy 2: Largest-error-under-1% (choose candidate with max abs error < 1%)
#
...
namespace eval rounding {
  # Default step set (edit if you like; adding {20 25 50 100} is fine)
...
# -------------------------------
proc rounding::round_grid_first_ge_1pct {x} {
  if {$x == 0.0} { return 0.0 }
  variable STEPS

  set ax     [expr {abs($x)}]
  set u      [rounding::_unit3sf $x]
  set onepct [expr {0.01 * $ax}]
  ...
  if { !($err <= $onepct + $eps) } {
    error "round_grid_first_ge_1pct: Error $err exceeds 1% of |x| ($onepct)"
  }
  return $y
}
...
# ------------------------------------------
proc rounding::round_largest_error_under_1pct {x} {
  if {$x == 0.0} { return 0.0 }
  variable STEPS
  ...
  if { !($err <= $onepct + $eps) } {
    error "round_largest_error_under_1pct: Error $err exceeds 1% of |x| ($onepct)"
  }
  return $y
}
