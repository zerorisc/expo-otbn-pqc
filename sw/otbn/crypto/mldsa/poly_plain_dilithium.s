...
poly_challenge:
    ...
    /* Adjust sp to accomodate local variables */
    /* NOTE: only allocate this much when SW Keccak */
    addi sp, sp, -1088

    /* Reserve space for tmp buffer to hold a WDR */
    #define STACK_WDR2GPR -32
    #define STACK_SHAKEBUF -64
    #define STACK_WDRSPILL -1088
...
_rej_sample_loop:
    /* First squeeze */
    /* Software Keccak */
    .irp reg,t0,t2,t3,t5,t6,a0,a1,a2,a3,a5,a6
        push \reg
    .endr
    ...
    /* Get the shake output into the shake REG */
    li a0, 8
    bn.lid a0, STACK_WDR2GPR(fp)

    .irp reg,a6,a5,a3,a2,a1,a0,t6,t5,t3,t2,t0
        pop \reg
    .endr
...
_aligned_poly_uniform_eta_base:
    ...
    #define STACK_WDRSPILL -96

    .irp reg,s0,s1,s2,s3,s4
        push \reg
    .endr
...
_rej_eta_base_sample_loop:    
    /* Software Keccak */
    .irp reg,t0,t3,t5,t6,a0,a1,a2,a5,a6
        push \reg
    .endr
    ...
    /* Get the shake output into the shake REG */
    li x10, 8
    bn.lid x10, STACK_WDR2GPR(fp)

    .irp reg,a6,a5,a2,a1,a0,t6,t5,t3,t0
        pop \reg
    .endr
 ...
 * @param[in]  a4: SHAKE context buffer
 *
...
poly_uniform_gamma1_base_dilithium:
    ...
    /* Reserve space for tmp buffer to hold a WDR */
    #define STACK_WDR2GPR -32
    /* Reserve space for holding spilled WDR */
    #define STACK_WDRSPILL -224

    .irp reg,a0,a1,a2,a3,a4
        push \reg
    .endr
    addi s1, a1, 0
    addi s2, a2, 0
    ...
    /* Send the seed to Keccak */
    addi a0, s4, 0
    addi x11, s1, 0
    li x12, CRHBYTES
    jal x1, sha3_update

    /* Send the nonce to Keccak */
    addi a0, s4, 0
    sw   s2, STACK_WDR2GPR(fp)
    addi a1, fp, STACK_WDR2GPR /* a1 <= *STACK_WDR2GPR = *nonce*/
    li x12, 2
    jal x1, sha3_update
