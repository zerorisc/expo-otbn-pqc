 ...
 *
 */
.globl verify_plain_dilithium
verify_plain_dilithium:
    ...
    #define STACK_SHAKE_CONTEXT -34976 /* Prev - 256 */
#elif DILITHIUM_MODE == 3
    ...
    #define STACK_SHAKE_CONTEXT -56512 /* Prev - 256 */
#elif DILITHIUM_MODE == 5
    ...
    #define STACK_SHAKE_CONTEXT -91584 /* Prev - 256 */
#endif
    ...
    /* Absorb PK into the Keccak state */
    li  x10, STACK_SHAKE_CONTEXT
    add x10, fp, x10
    addi x11, s0, 0
...
/* -------------------------------------------------------------------------- */
    ...
    /* Absorb mu into the Keccak state */
    li  x10, STACK_SHAKE_CONTEXT
    add x10, fp, x10
    la x11, STACK_MU
    add x11, fp, x11
    ...
    /* Absorb msg into the Keccak state */
    li  x10, STACK_SHAKE_CONTEXT
    ...
    li x12, 0
    LOOPI K, 1
        addi x12, x12, POLYW1_PACKEDBYTES
    jal x1, sha3_update
