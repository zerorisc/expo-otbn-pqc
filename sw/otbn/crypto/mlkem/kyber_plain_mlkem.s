...
indcpa_keypair_plain:
  ...
  jal  x1, sha3_init
  la   a0, context
  lw   a1, STACK_COINS_ADDR(fp)
  li   a2, 32 /* input len */
  jal  x1, sha3_update
  addi  a1, zero, KYBER_K
  sw    a1, STACK_BUF(fp)
  la    a0, context
  addi  a1, fp, STACK_BUF
  addi  a2, zero, 1
  jal   x1, sha3_update
  la   a0, context
  addi a1, fp, STACK_BUF
...
crypto_kem_keypair: 
  ...
  jal  x1, sha3_init
  la   a0, context
  lw   a1, STACK_PK_ADDR(fp)
  li   a2, KYBER_PUBLICKEYBYTES
  jal  x1, sha3_update
  la   a0, context
  lw   a1, STACK_SK_ADDR(fp)
  addi a1, a1, KYBER_INDCPA_PUBLICKEYBYTES
  addi a1, a1, KYBER_INDCPA_SECRETKEYBYTES
  jal  x1, sha3_final
...
crypto_kem_enc: 
  ...
  jal  x1, sha3_init
  la   a0, context
  lw   a1, STACK_KEM_ENC_PK_ADDR(fp)
  li   a2, KYBER_PUBLICKEYBYTES
  jal  x1, sha3_update
  la   a0, context
  li   a1, STACK_KEM_ENC_BUF
  add  a1, fp, a1 
  addi a1, a1, 32
  jal  x1, sha3_final
  ...
  jal  x1, sha3_update
  la   a0, context
  lw   a1, STACK_KEM_ENC_KEYB_ADDR(fp)
  jal  x1, sha3_final

  /*** indcpa_enc ***/
  li  a0, STACK_KEM_ENC_BUF
  add a0, fp, a0 
...
crypto_kem_dec:
  ...
  jal  x1, sha3_init
  la   a0, context
  li   a1, STACK_KEM_DEC_BUF
  add  a1, fp, a1
  li   a2, 64
  jal  x1, sha3_update
  la   a0, context
  li   a1, STACK_KEM_DEC_KR
  add  a1, fp, a1 
  jal  x1, sha3_final

  /*** indcpa_enc ***/
  li   a0, STACK_KEM_DEC_BUF
  add  a0, fp, a0
  ...
  la   a0, context
  lw   a1, STACK_KEM_DEC_H_ADDR(fp)
  addi a1, a1, 32 
  li   a2, 32
  jal x1, sha3_update

  la   a0, context
  lw   a1, STACK_KEM_DEC_CT_ADDR(fp)
  li   a2, KYBER_CIPHERTEXTBYTES
  jal  x1, sha3_update
